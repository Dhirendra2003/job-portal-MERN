...
const ChattingList = ({isChatOpen,setIsChatOpen}) => {
  ...
  return (
    <div className="pt-3 flex flex-col h-[60vh]  overflow-auto ">
      {chatList.length>0 && chatList.map((item,index)=>{
        return (
          <div onClick={()=>setIsChatOpen(true)} key={index} className={`grid grid-cols-3 p-4 h-20 border-neutral-300 border-b-[1px] bg-${item.unreadmsg? "purple-100":'white'} items-center `}>
            <img src={item.logo} alt="" className='m-auto shadow-md rounded-lg w-12 h-12 ' />
            <h3 className='col-span-2 pl-3 text-xl flex'>{item.company}
